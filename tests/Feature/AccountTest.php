...
class AccountTest extends AbstractTestCase
{
    ...
    public function testSuccessLogin()
    {
        ...
        $permanentId = '1'; // factoryにて登録済みのQiitaアカウントのID
        ...
        // 実際にJSONResponseに期待したデータが含まれているか確認する
        $accountId = 3; // RefreshDatabaseが実行されてautoincrementの値はリセットされない
        $expectedEmbedded = [
            'sessionId' => $responseObject->_embedded->sessionId
        ];
        $jsonResponse->assertJson(['accountId' => $accountId]);
        $jsonResponse->assertJson(['_embedded' => $expectedEmbedded]);
        $jsonResponse->assertStatus(201);

        // DBのテーブルに期待した形でデータが入っているか確認する
        $idSequence = 3; // RefreshDatabaseが実行されてautoincrementの値はリセットされない
